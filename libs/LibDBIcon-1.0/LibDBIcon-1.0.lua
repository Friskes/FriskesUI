...
local function updatePosition(button)
	local angle = math.rad(button.db and button.db.minimapPos or button.minimapPos or 303)
...
local function createButton(name, object, db)
	...
	if IsAddOnLoaded("FriskesUI") then
		overlay:SetVertexColor(FuiColor.R, FuiColor.G, FuiColor.B, FuiColor.A)
	else
	...
	icon:SetWidth(16.5); icon:SetHeight(16.5)
	icon:SetTexture(object.icon)
	icon:SetTexCoord(0.05, 0.95, 0.05, 0.95)
	icon:SetPoint("TOPLEFT", 7, -4.7)
	button.icon = icon
